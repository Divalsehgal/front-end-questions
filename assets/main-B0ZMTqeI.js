...
.inline-container {
  display: inline-flex;
}`,text:"<code>display: flex</code> creates a block-level container (full width). <code>display: inline-flex</code> creates an inline-level container (content width)."},{title:"&lt;div&gt; vs &lt;section&gt;",code:`&lt;!-- Non-semantic --&gt;
&lt;div&gt;
  &lt;h2&gt;My Articles&lt;/h2&gt;
&lt;/div&gt;
...
&lt;section&gt;
  &lt;h2&gt;My Articles&lt;/h2&gt;
&lt;/section&gt;`,text:"<code>div</code> is a generic, non-semantic container with no inherent meaning, used for layout and grouping. <code>section</code> is a semantic HTML5 element that groups related content with a heading, helping screen readers navigate content structure. Semantic HTML improves accessibility, enhances SEO, and creates a more meaningful document structure."}],l=[{title:"1. The Main HTML Skeleton",description:"Every modern web page starts with a foundational skeleton. Below is the exact HTML structure we use for this project hub. The following sections break down the 'why' behind each block of this code.",code:`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    <!-- Application Content -->
        ...
        `,c.appendChild(t)}),document.addEventListener("click",e=>{if(e.target.classList.contains("copy-btn")){const t=e.target,o=t.parentElement.querySelector("code").innerText;navigator.clipboard.writeText(o).then(()=>{t.innerText,t.innerText="Copied!",t.classList.add("copied"),setTimeout(()=>{t.innerText="Copy",t.classList.remove("copied")},2e3)}).catch(i=>{console.error("Failed to copy text: ",i),t.innerText="Error"})}}),document.querySelectorAll(".k-card").forEach((e,t)=>{e.style.opacity="0",e.style.transform="translateY(20px)",e.style.transition=`all 0.5s ease ${t*.1}s`,setTimeout(()=>{e.style.opacity="1",e.style.transform="translateY(0)"},100)});const a=document.getElementById("open-wiki-btn"),n=document.getElementById("wiki-modal");if(a&&n){const e=document.getElementById("wiki-content-container");e&&typeof l<"u"&&l.forEach(o=>{const i=document.createElement("div");i.className="wiki-section";let r=`
            <h3>${o.title}</h3>
            <p>${o.description}</p>
          `;if(o.code){const h=o.code.replace(/</g,"&lt;").replace(/>/g,"&gt;");r+=`
               <div class="code-block" style="margin-top: 1rem;">
                 <pre><code>${h}</code></pre>
                 <button class="copy-btn">Copy</button>
               </div>
             `}i.innerHTML=r,e.appendChild(i)});const t=n.querySelector(".modal-close");a.addEventListener("click",()=>{n.classList.add("open"),n.setAttribute("aria-hidden","false"),document.body.style.overflow="hidden"});const s=()=>{n.classList.remove("open"),n.setAttribute("aria-hidden","true"),document.body.style.overflow=""};t.addEventListener("click",s),n.addEventListener("click",o=>{o.target===n&&s()}),document.addEventListener("keydown",o=>{o.key==="Escape"&&n.classList.contains("open")&&s()})}});
