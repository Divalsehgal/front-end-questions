...
For the underlying theory behind these questions, see [rendering-cycle/README.md](./rendering-cycle/README.md) (render/commit phases) and [javascript/core-language/closure/index.md](../javascript/core-language/closure/index.md) (why stale closures happen — most of the `useState`/`useEffect` questions here are that same bug in different shapes). To see the same hook patterns applied in real components rather than quiz snippets, try `machine-coding/react-projects` challenges like `reducer-todo` (useReducer) and `infinite-reloading` (useEffect + refs).
...
| 5 | B | Even inside `setTimeout` (React 18's automatic batching applies broadly), both `setCount(count+1)` use the same stale `count` closure, so both compute `0+1=1`; batched into one render showing `1`. |
...
| 11 | B | Classic stale closure bug: the effect's `count` is captured once at mount (`0`) and never updates, so `setCount(0 + 1)` fires repeatedly, always setting it back to `1` — the display stays stuck at `1` forever, never incrementing further. |
...
| 21 | B | Nothing here triggers a re-render, so `App` only ever runs once (mount): `count` is a plain local variable captured by `handleClick`'s closure, and since that same closure instance is reused for every click (no re-render recreates it), `count += 1` keeps incrementing normally across clicks: 1, 2, 3. `countRef.current` was set once from that same initial `count` (`0`) and is never reassigned anywhere, so it stays `0` forever. |
...
| 30 | B | Unlike `useState`, `useReducer`'s dispatch always operates against the latest state maintained internally by React, not a stale closure — so sequential dispatches in the same batch correctly compound: `0→1→2`. |
