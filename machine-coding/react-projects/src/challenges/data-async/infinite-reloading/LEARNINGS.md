...
### 📦 Variant: Scrolling Inside a Container (not the window)
If the scrollable area is a `<div>` instead of the page itself, pass that div as the observer's `root` — otherwise the observer watches the viewport and never fires while the div scrolls internally.

```javascript
const containerRef = useRef(null);

const observer = new IntersectionObserver(callback, {
  root: containerRef.current, // observe relative to this div, not the viewport
  threshold: 1,
});
```
