...
const Otp: React.FC<OtpProps> = ({ length = 6 }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 bg-surface-sunken rounded-[3rem] min-h-[450px]">
        ...
        {otp.map((value, index) => (
          <div key={index} className="relative group">
             <input
              ...
              value={value}
              className={cn(
                "w-12 h-16 md:w-16 md:h-20 bg-surface border-2 border-border-strong rounded-2xl text-center text-3xl font-mono font-black",
                "focus:border-primary focus:ring-8 focus:ring-primary/10 outline-none transition-all duration-300",
            ...
            <div className={cn(
               ...
               value ? "bg-primary w-8" : "bg-border-strong group-hover:bg-primary/30"
            )} />
          </div>
        ))}
      </div>
      ...
      <div className="mt-16 group">
        <button 
          ...
          className={cn(
            ...
            isComplete 
              ? "bg-primary text-white shadow-2xl shadow-primary/30 hover:scale-105 active:scale-95 cursor-pointer" 
              : "bg-surface border-2 border-border-strong text-text-muted opacity-50 cursor-not-allowed"
          )}
        >
      ...
      <p className="mt-8 text-sm text-text-muted hover:text-primary transition-colors cursor-pointer">
        Didn't receive code? <span className="font-bold underline decoration-primary/30 underline-offset-4">Resend OTP</span>
      </p>
    </div>
  );
};
