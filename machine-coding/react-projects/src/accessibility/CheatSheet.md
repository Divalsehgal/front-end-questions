...
Accessibility is all about making sure web elements are understood properly by assistive technologies (like screen readers). When building custom components, semantic native HTML isn't always enough, so ARIA (Accessible Rich Internet Applications) attributes are used to bridge the gap.

---

## 1. Forms: Linking Labels (`htmlFor` ↔ `id`)
This is the most common accessibility pattern. It connects a text label to an input. When standard users click the label, the input gets automatically focused. For screen readers, it announces the input's name properly.

**When to use:** Always, when building forms.
**How it works:** `htmlFor` on a `<label>` must match the `id` on the `<input>`, `<select>`, or `<textarea>`.
...
### A. `aria-controls` ↔ `id`
**What it does:** Tells the screen reader that interacting with the current element will affect or control *another* element.
**When to use:** Tabs controlling Panels, Accordion Buttons controlling Accordion Bodys, Menus controlling Dropdown Lists.
...
**What it does:** Replaces the element's accessible name with the text content of another element.
**When to use:** Modals/Dialogs taking their title from an internal heading, or Tab Panels getting their names from the Tab buttons.
...
**What it does:** Provides supplementary descriptive text (like hints or validation errors) to a primary element.
**When to use:** Input hint texts, form validation error messages.
...
> **Important Rule of Thumb:** If semantic HTML already exists (e.g., `<button>`, `<nav>`, `<input>`), use it! Do NOT use a `<div>` with `role="button"` unless absolutely necessary. Semantic HTML gives you keyboard support (Tab, Enter, Space) out of the box!
...
*Use these for interactive patterns that don't have a single native HTML equivalent.*
...
- **Interaction Rule**: In a `tablist`, you must manually manage keyboard focus using arrow keys! (Left/Right to move between tabs).
...
Remember the classic `Tabs` pattern. It uses roles for structure, and IDs for relationships!
